...
const Model = ({ setisModelLoading }) => {
  ...
  useEffect(() => {
    if (ismobile !== null) {
      ...
      const distance = 10;
      const frustumHeight =
        2 * Math.tan(THREE.MathUtils.degToRad(camera.fov) / 2) * distance; // Vertical size of the camera's frustum at the given distance
      ...
      let positionArr = [
        ...
        },
        {
          id: "projects",
          position: {
            x: Math.min(objectPositionX - (ismobile ? 0.15 : 0.5), 3),
            y: -0.5,
            z: 0,
          },
